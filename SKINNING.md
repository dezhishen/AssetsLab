...
### 5. 皮肤可替换（已实现）
- 皮肤 = 独立**皮肤包** `skins/<name>/`（`skin.json` + 标准命名部件 `<NNN>_<layer>_<view>.png` + `preview/` 预览动画），**不在制品里**；序号按**身体区域分段**（3 位）：`000`头颈 / `100`左臂 / `200`右臂 / `300`躯干 / `400`左腿 / `500`右腿 / `600`脚（每区预留 100 槽）
