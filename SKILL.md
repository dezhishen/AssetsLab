...
python -m workflow new --definition default --id hero --template cartoon   # 新建（可带风格模板）
python -m workflow status --workflow hero --json                   # 状态 + template + 各动作 params
...
**参数优先级**：运行时 `--param` > 实例模板 `template_params` > 定义默认值。
...
**运动参数**（每动作）：`stride` 步幅、`pelvis_bob` 骨盆起伏、`arm_swing` 摆臂。
**体型比例**（骨骼基座）：`arm_length`/`leg_length`/`torso_length`/`shoulder_width`/`head_scale`/`height`（1.0=基准），作用于静态基座，同一动作可驱动任意体型。
...
- **`flow.html?id=<workflow_id>`**：分步流程向导（上一步/下一步，每步调参→运行→评审）
- **API**（前缀 `http://127.0.0.1:8765`）：
  - `GET /api/workflow/list|templates|definitions`
  - `GET /api/workflow/instances/<id>` / `.../next`
  - `POST /api/workflow/instances` （body: definition/id/template）
  - `POST /api/workflow/instances/<id>/actions/<action_id>/{run|approve|reject}`（run 可带 `params`）
...
1. `new --template <风格>` 建实例 → `status` 确认 template_params
2. `next` 取推荐动作 → `run`（可 `--param` 调 stride/bob/arm_swing 或比例）
3. `status --json` 看该动作 `outputs`（本地图片绝对路径）+ `params`（实际所用参数）
4. 满意 `approve`（带 note）→ 不满意 `reject`（附调整建议）→ 重新 `run --param`
...
- 新动作预设=新 JSON（`workflow/motions/`）；新风格模板=新 JSON（`workflow/templates/`）。
- 调整参数时优先考虑模板风格；微调用 `--param`；改体型用 `--proportion-*`。
