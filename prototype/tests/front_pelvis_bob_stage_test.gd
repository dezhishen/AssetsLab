...
const MODEL := preload("res://scripts/front_pelvis_bob_stage.gd")
const OUTPUT_DIRECTORY := "res://test_output/skeleton_pipeline/front_pelvis_bob"
...
func _run() -> void:
	...
	await process_frame
	var errors := model.validate_pelvis_bob()
	...
	for index in range(8):
		...
		if image.save_png(output_path) != OK:
			...
			return
	print("FRONT_PELVIS_BOB_STAGE_PASS frames=8 feet=stage2_unchanged pelvis=vertical_only output=%s" % absolute_directory)
	quit(0)


func _fail(message: String) -> void:
	push_error("FRONT_PELVIS_BOB_STAGE_FAIL: " + message)
	quit(1)
