...
const MODEL := preload("res://scripts/front_arm_swing_stage.gd")
const OUTPUT_DIRECTORY := "res://test_output/skeleton_pipeline/front_arm_swing"
...
func _run() -> void:
	...
	await process_frame
	var errors := model.validate_arm_swing()
	...
	for index in range(8):
		...
		if image.save_png(output_path) != OK:
			...
			return
	print("FRONT_ARM_SWING_STAGE_PASS frames=8 legs=stage3_unchanged arms=opposite_phase output=%s" % absolute_directory)
	quit(0)


func _fail(message: String) -> void:
	push_error("FRONT_ARM_SWING_STAGE_FAIL: " + message)
	quit(1)
