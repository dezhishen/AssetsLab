...
argument list.  This is the pure-Python mirror of the resolution rules in
``tools/resolve_godot.ps1`` and ``tools/resolve_python.ps1`` so that the same
workflow runs on Windows, Linux and macOS.  The existing PowerShell scripts
remain the canonical Windows entry points; the cross-platform CLI
(``tools/assetslab.py``) builds on this module.
...
def resolve_python(requested: str | None = None) -> str:
    ...
    Resolution order (mirrors ``resolve_python.ps1``):
...
def resolve_godot(requested: str | None = None) -> str:
    ...
    Resolution order (mirrors ``resolve_godot.ps1``):
